...
void xefi_init_serial(void) {
    efi_loaded_image_protocol* loaded;

    efi_status r = xefi_open_protocol(gImg, &LoadedImageProtocol, (void**)&loaded);
    if (r) {
        printf("LoadFile: Cannot open LoadedImageProtocol (%s)\n", xefi_strerror(r));
        goto exit0;
    }
    efi_serial_io_protocol* serial;
    r = xefi_open_protocol(loaded->DeviceHandle, &SerialIoProtocol, (void**)&serial);
    if (r) {
        printf("LoadFile: Cannot open SerialIoProtocol (%s)\n", xefi_strerror(r));
        goto exit1;
    }
    gSerialOut = serial;
exit1:
...
void xefi_free(void *data, size_t size) {
